...
3. **写系统层（快捷键 / 注入 / 流式）**：先去看 §8 列出的 Handy 对应实现，按它的模式来，不要凭空生成。
...
| P0.3 | 松手 → 调用 Groq `whisper-large-v3-turbo` → 控制台打印文字 | 控制台拿到正确文字 |
| P0.4 | 剪贴板法注入光标处（保存→写入→粘贴→恢复） | 任意输入框拿到文字 |
| P0.5 | 设置持久化（`tauri-plugin-store`） | 重启后快捷键保留（麦克风选择推到 P3 设置页一起做） |
| P0.6 | 错误态最朴素呈现（无权限 / 网络失败胶囊变红） | 麦克风权限拒绝 → 红胶囊 |
| P0.7 | 错误态全大类 + 输入设备智能挑选 | Inject/Device/Network/Provider 四大类都能上红胶囊；连 AirPods/iPhone 接力麦时自动用内置/USB |

> **录音起止规则**：松手（push-to-talk）是 P0 唯一的结束判定。**不做 VAD 自动断句**——产品方向是 P3 的 fn 单击 toggle 手动控制起止 + P2 流式边录边传，与「系统自动决定何时结束」对立。当前按住快捷键是临时过渡方案。
...
> 取消路径（CANCEL 状态）P0 留空骨架不实装——产品决定换成 P3 的「胶囊 ✓/✗ 按钮 + toggle 触发 + 可撤回」交互模型，见 §5 P3「Overlay 交互 + 控制模型」。`AppState::Cancel` 和 `Recording→Cancel`/`Processing→Cancel`/`Cancel→Idle` 三条合法转移保留在状态机里，P3 重新填路径时不用改 enum。
...
```
IDLE ──hotkey-down──▶ RECORDING ──hotkey-up──▶ PROCESSING ──ok──▶ SUCCESS ──150ms──▶ IDLE
                                                            │
                                                            └──err──▶ ERROR ──2.5s──▶ IDLE
```

P0 还有两条 Idle→Error 转移用于按键前的 permission/device gate（麦克风权限拒、cpal 设备失败），从 ERROR 也回 IDLE。`Cancel` 状态保留但 P0 无入口（见 §3.2 注）。
...
| `partial-transcript` | 类型已定义但暂不 emit（见 §5.2）；未来做 partial UI 时启用 | `text`, `is_final`, `sequence` |
...
|---|---|---|---|
| IDLE | 不显示 | — | — |
| RECORDING | 胶囊淡入，只显示跳动波形（P2.4 起移除「录音中…」文字；产品方向不做 partial 滚动字幕，见 §5.3） | `audio-level`（波形） | 上滑淡入 120ms |
...
- [ ] 按全局快捷键胶囊出现，松开消失（P0.1+P0.2）
- [ ] 录音时胶囊里波形跟着说话音量动（P0.2）
- [ ] 控制台能看到 Groq 返回的正确文字（P0.3）
- [ ] 在 VS Code、Chrome 地址栏、微信都能正确粘贴（P0.4）
- [ ] 重启 App 后快捷键还在（P0.5）
- [ ] 拒掉麦克风权限 → 胶囊变红 + 看得到提示（P0.6）
- [ ] 取消辅助功能权限 → 红胶囊 + 转写文字仍在剪贴板可手动粘贴（P0.7 §3.7 Inject fallback）
- [ ] 关代理在国内 → 红胶囊「Groq 拒绝请求（可能地区限制）」（P0.7 §3.7 Network/Provider）
- [ ] AirPods 是系统默认输入时录音正常（自动切到内置麦，P0.7）

> Esc 中断录音 / 麦克风设备选择持久化已从 P0 移除，挪进 P3「Overlay 交互 + 控制模型」与「设备选择切片」一起做。
...
> **产品方向更正（2026-06，读完 Voxt 豆包实现后定）**：用户要的不是「边说边看滚动字幕」，而是 typeless 同款「按住说一大段 → 松手几百毫秒内出 final」。所以 **P2 不做 overlay 实时 partial 字幕**，流式的价值兑现在「边录边传让 final 更快」上。`partial-transcript` event 类型在 P2.2 已定义但 P2 不 emit，留给未来真要做 partial UI 时启用。第一个流式 provider 选**豆包（火山引擎大模型流式 ASR）**，国内可直连，绕开 Groq 在大陆 403 的问题。

### 5.1 P2 切片清单（每个独立 commit，不收尾不开下一个）

> 原 P2.3「一片接完整流式」太大（codec + WebSocket + 配置 UI + 音频管线四件不同性质的事），违反「一片 ≤ 90 分钟」原则，拆成 P2.3–P2.6 四片，加收尾共五片。完整方案见 `~/.claude/plans/gentle-churning-owl.md`。

| 切片 | 内容 | 验收 |
|---|---|---|
| P2.1 | P2 正式 SPEC：接口合同 + 切片表 | 本节切片、验收、command/event/provider 语义就位；`CLAUDE.md` 当前进度同步 |
...
| P2.3 | 豆包二进制 codec + 单测（纯函数，不连网/不动 manager/不动 UI） | `asr/doubao/codec.rs` 能 build full client request / audio chunk / final negative，parse server response（含 gzip 解压 + `result.text` 抽取 + 错误帧 → Provider error）；round-trip + 错误分支单测全绿；新依赖 `flate2` |
| P2.4 | 豆包配置 UI + keychain + overlay 文案清理（不连网/不接录音） | 设置页能填 AppID / Access Token / endpoint / resource_id，Token 走 keychain（key id `doubao_access_token`）；重启后 Token 仍在 keychain、导出 JSON 不含明文；overlay RECORDING 态去掉「录音中…」占位文字只剩波形；**不进 `list_asr_providers`** |
| P2.5 | AudioManager PCM chunk 实时出口 + dev-only 离线连通性 command | cpal callback 多一路 chunk 广播（批量 buffer 不变）；`asr/doubao/client.rs` ws 客户端；dev command `test_doubao_streaming(wav_path)` 喂本地 16k mono wav → Rust 日志能看到豆包 partial/final；批量录音路径未被破坏；新依赖 `tokio-tungstenite` + `tokio-rustls` |
| P2.6 | 录音热路径接流式（临时开关控制，不动 overlay） | 开关开 + 豆包配好 → 录音时并行送豆包 ws，松手用豆包 final 走现有润色/注入链路；开关关或豆包未配 → 退化为 P1 批量；流式失败按 §3.7 归类且不影响后续 hotkey；**不 emit `partial-transcript`、不改 overlay** |
| P2.7 | P2 收尾：默认走流式 + SPEC 同步 + 延迟测量 | 豆包配好默认走流式、未配批量降级，全程无需手动操作；P2.6 临时 preview 开关退役；连续 5 次录音延迟数据落档（§5.4）；§5.1 与现实一致；北极星「感知延迟 ≈ 1s 与说话时长无关」达标 |

### 5.2 P2 接口边界

- `AsrProvider` 在 P2.2 扩展 `transcribe_stream` 合同：接收音频 chunk 流，输出统一的 partial/final transcript 流。各家 WebSocket 协议差异只允许收敛在 adapter 内，不向 pipeline / overlay 泄漏。豆包的二进制帧 codec 收敛在 `asr/doubao/`。
- `partial-transcript`（§3.6）：类型保留，**P2 不 emit**。流式期间 overlay 维持「只波形、无文字」，partial 只在 P2.5 的 dev command 里打日志辅助调试。`text` / `is_final` / `sequence` 语义留给未来要做 partial UI 时再启用。
- `final-transcript` 继续表示最终可注入文本。P2 的流式只是让 final 更快到手（边录边传），注入仍是松手后一次性，不做真实「流式注入到输入框」。
...
### 5.2.1 流式 Provider 代码规范

- 新 provider 开工前必须记录官方文档 URL、endpoint、auth 方式、音频格式/采样率/chunk size、partial/final 判断方式、错误码或握手排查字段（如 logid/request id）。官方文档优先级高于第三方示例；Voxt 这类现有实现只能作为参考。
- provider 目录固定为 `asr/{provider}/`：`config.rs` 放官方常量/默认值/key id，`client.rs` 放鉴权、WebSocket/HTTP 客户端和错误分类；有私有 wire protocol 时再加 `codec.rs`，且 codec 只能做 encode/decode，不读设置、不联网。
- 鉴权和厂商 header 只能收敛在 provider client/config 内，不向 `TranscriptionManager`、pipeline、overlay 或设置 UI 泄漏；UI 只表达用户能理解的字段，旧版/新版控制台兼容由 provider 内部模型承接。
- `commands.rs` 只允许放 dev-only 探针和参数组装，不放厂商协议逻辑；真实热路径只接统一 `AsrProvider::transcribe_stream` 合同。
- 每个 provider 必须有不联网单测覆盖默认配置、鉴权请求构造、错误分类；有 `codec.rs` 时必须覆盖 round-trip 和错误帧。联网验证只走 dev command 或手动 smoke，不进普通单测。

### 5.3 P2 明确不做

- **Overlay 实时 partial 滚动字幕**：产品方向反转，要的是「松手即出 final」不是边看字幕。P2 overlay RECORDING 态只显示波形。真要做 partial UI 留到以后单独立项。
- fn 单键、toggle 触发、Esc 中断、✓/✗ 按钮：留到 P3「Overlay 交互 + 控制模型」。P2 仍用「按住 `Ctrl+Shift+Space`」过渡。
- 麦克风设备下拉、设备 UID 持久化、设备变化监听：留到 P3「设备选择切片」。
- 零配置默认 ASR、本地模型下载、onboarding：留到 P3。
- 多个流式 provider 同时实现：P2 只把豆包一个跑通，第二个（阿里 Paraformer / Deepgram）等以后。
- 豆包流式进 `list_asr_providers` 成为可选批量 provider：P2 它只走流式热路径，不当批量 ASR 选项。

### 5.4 P2 能力与风险

**关键能力**：
- ASR provider 加 `transcribe_stream` 方法，走 WebSocket（P2 落地豆包；阿里 Paraformer-realtime / Deepgram 留作以后扩展）
- 录音边采集边上传 chunk
- 按下快捷键即预热连接（消除冷启动）

**关键风险**：
- 不同流式 provider 协议差异大，trait 抽象要扛得住（豆包是自定义二进制帧，不是 JSON over WS）
- 流式期间 cpal callback 要同时喂「批量 buffer」和「chunk 广播」两个消费者，不能互相阻塞
- 注入策略不变：松手后一次性注入，流式只负责让 final 更快

**延迟测量（P2.7 落档）**：
- 代码策略：豆包 `doubao_access_token` 已配置时默认走流式；未配置时自动走当前批量 ASR provider。
- 连续 5 次「松手 → 注入完成」实测延迟：_待手动验收填入（需要真实 App、豆包 keychain key、目标输入框和人工观察注入完成时间）_

### P3 — 体验打磨

P3 从这里开始进入正式拆片阶段。P3 的目标不是一次性"美化整个 App"，而是把 typeless 手感拆成可验收的小步：先做 Audie 自己的组件库，再基于组件库打磨 overlay / 设置页 / onboarding，最后补设备 escape hatch、控制模型、fn、词典和上下文。

**设计资产**：Figma `Audie Design System v1`：https://www.figma.com/design/HwyjD0TlY1amuLcqfCvPNB

### 5.5 P3 切片清单（每个独立 commit，不收尾不开下一个）

| 切片 | 内容 | 验收 |
|---|---|---|
| P3.0 | P3 正式 SPEC + UI/UX SPEC | 本节 P3 切片表、UI/UX 边界、interaction 边界、command/event 边界、明确不做范围就位；`CLAUDE.md` 当前进度同步；只改文档，不写代码、不引入新库 |
| P3.1 | Design System / Component Library v1 | Figma 有 Foundations / Core Components / Product Components / Patterns / Screens Drafts；`design.dark.md` / `design.md` 的 Geist colors / typography / spacing / rounded / component tokens 已全量整理进 Foundations；组件库暗色优先，浅色保留 token；不接 `daisyui-blueprint`，不复制付费 Blueprint 内容 |
| P3.2 | Overlay 胶囊视觉打磨 | 基于组件库实现 RECORDING / PROCESSING / SUCCESS / ERROR / CANCEL 五态视觉和动效；不改录音、转写、注入业务逻辑；状态仍只由 Rust event 驱动 |
| P3.3 | 设置页信息架构重排 | 基于组件库重排 Provider / Hotkey / Device / Enhance / Import-export 分区；现有设置可读写；不新增业务能力 |
| P3.4 | 设备选择 escape hatch | 设置页能列麦克风、选择输入设备、重启保留；未选择时保留 P0.7 自动挑设备；选中设备不可用时走 `Device` 错误 |
| P3.5 | Toggle 录音控制 | 现有快捷键支持"按一次开始、再按一次结束"；录音热路径仍走现有流式/批量、润色、注入链路；不接 fn |
| P3.6 | Overlay 可交互控制模型 | 胶囊有可点击的确认/取消入口；取消不注入，转录文本仍可复制/撤回；按钮区可点，其余区域尽量保持不干扰当前 App |
| P3.7 | Esc 中断并入新控制模型 | Esc 行为与 P3.6 的取消语义一致；不复用 P0 砍掉的 Handy 风格 Esc 实现 |
| P3.8 | fn / 双击 Option 输入监听探针 | 用最小 dev-only 探针验证 CGEventTap + Accessibility + Input Monitoring 能收到 fn / 双击 Option；不替换主触发键 |
| P3.9 | fn / 双击 Option 正式触发 | 设置页可选触发方式；默认仍可回退组合键；缺权限走 `Permission` 错误并给引导 |
| P3.10 | 个人词典 v1 | 设置页能维护术语；润色 prompt 注入术语；vibe coding 常用词不被乱改 |
| P3.11 | App 上下文 prompt v1 | macOS 下按前台 App 给 LLM 加轻量上下文；IDE 偏代码、聊天偏口语；没有前台 App 信息时退回默认 prompt |
| P3.12 | Onboarding / 权限引导 v1 | 基于组件库做首次启动流程；不看 README，5 分钟内能完成权限、provider key、测试录音、一次注入；失败路径有明确下一步 |
| P3.13 | P3 收尾验收 | 完整跑 P3 验收清单；`CLAUDE.md` 当前进度更新；确认没有偷偷长出 Non-Goals |

### 5.6 P3 UI/UX 边界

- UI 设计并入 P3 切片体系，不单独开一个平行模块。先组件库，再 screens，避免 overlay、设置页、onboarding 各画各的。
- 组件库顺序固定为 Foundations → Core Components → Product Components → Patterns → Screens Drafts。
- Foundations 以 `design.dark.md` / `design.md` 为 source of truth：96 个 sRGB color token 写成 Figma Light/Dark 变量；70 个 Display P3 OKLCH token 先作为文档引用；30 个 typography token 写成 text styles；spacing / rounded / component tokens 写入 Foundations 文档。若 Figma 环境缺 Geist Sans / Geist Mono，text style 可暂时回退 Inter，但 token 名称和字号/行高/字距必须保留。
- Overlay 是 P3 第一优先的产品视觉面，但必须基于组件库落地，不把 partial 滚动字幕加回来。
- 设置页先重排信息架构，再逐步接设备、触发方式、词典、onboarding。每片只改对应区域，不做跨页重写。
- Onboarding 是 P3 后段工作：它依赖权限、provider、触发方式和设置结构稳定后再做。

### 5.7 P3 Interaction 边界

- 控制模型按顺序推进：组合键 toggle → overlay 可交互按钮 → Esc 并入取消语义 → fn / 双击 Option 探针 → fn / 双击 Option 正式触发。
- fn 不先于 toggle。fn 只是更顺手的触发入口，不应该同时承担控制模型设计。
- P3 的取消语义跟 P0 不同：取消不再只是丢弃，而是进入"不注入，但转录文本仍可复制/撤回"的模型。
- 改成可点胶囊后，需要重新审视 §3.8 的点击穿透：按钮区可点，其余区域尽量不干扰当前 App。

### 5.8 P3 Command / Event 边界

- P3.0 只声明未来会触碰的接口边界，不在本切片定最终 Rust 类型。
- `list_microphones` 已在 §3.5 占位，P3.3 实现时再补返回字段和设置字段。
- Toggle、Esc、fn、overlay 按钮都会影响 command/event 和状态机；进入对应切片前必须在本节补清楚具体合同，再写代码。
- 新 event 只在 Rust manager 发出，前端继续只镜像状态，不持有录音/转写业务逻辑。

### 5.9 P3 明确不做

- 不做会议转录 / 长音频 / 字幕导出。
- 不做语音命令 / agent / 执行操作。
- 不做实时翻译、说话人分离、账号体系、云同步、计费或自建后端。
- 不做多 provider priority/routing；如果以后要做，先补 SPEC 另开切片。
- 不做 overlay 实时 partial 滚动字幕；P3 继续坚持 P2 定下的"松手即出 final"方向。
...
| 项目 | 用途 | 怎么用 |
|---|---|---|
| **cjpais/Handy** | 主骨架，照着复刻 | clone 跑起来，读 `AGENTS.md` + `src-tauri/src/lib.rs` + `managers/`。写快捷键/注入时回去抄模式 |
