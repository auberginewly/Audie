...
| P2.4 | 豆包配置 UI + keychain + overlay 文案清理（不连网/不接录音） | 设置页能填 AppID / Access Token / endpoint / resource_id，Token 走 keychain（key id `doubao_access_token`）；重启后 Token 仍在 keychain、导出 JSON 不含明文；新增 `doubao_streaming_preview_enabled: bool`（默认 false）；overlay RECORDING 态去掉「录音中…」占位文字只剩波形；**不进 `list_asr_providers`** |
...
| P2.6 | 录音热路径接流式（开关控制，不动 overlay） | 开关开 + 豆包配好 → 录音时并行送豆包 ws，松手用豆包 final 走现有润色/注入链路；开关关或豆包未配 → 退化为 P1 批量；流式失败按 §3.7 归类且不影响后续 hotkey；**不 emit `partial-transcript`、不改 overlay** |
| P2.7 | P2 收尾：默认走流式 + SPEC 同步 + 延迟测量 | 豆包配好默认走流式、未配批量降级，全程无需手动操作；连续 5 次录音延迟数据落档（§5.4）；§5.1 与现实一致；北极星「感知延迟 ≈ 1s 与说话时长无关」达标 |
...
**延迟测量（P2.7 落档）**：
- 连续 5 次「松手 → 注入完成」实测延迟：_待 P2.7 填_
