...
**现在在做：** P3 前的手动验收：用真实 App + 豆包 key 连续 5 次记录「松手→注入完成」延迟，填回 `PROJECT_SPEC.md` §5.4；不要顺手开多 provider priority/routing，那个要先补 SPEC 另开切片。

> P2.3–P2.7 切片表已按「松手即出 / 不要滚动字幕 / fn 留 P3」同步到 `PROJECT_SPEC.md` §5.1。P2.7 代码收口完成，但真实延迟数据还需要手动跑 App 后落档。
