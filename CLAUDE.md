...
- macOS keychain 实现参考 Voxt：用底层 `SecItemCopyMatching` / `SecItemAdd` / `SecItemUpdate` 管 generic-password item，service 用 bundle id 派生，写入设置 `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly`；保存一次后重启应静默读取，不能每次读 key 都弹系统密码。
...
**现在在做：** P2.1 真流式 ASR provider 设计拆片（进入前先细化 SPEC §5 P2）。
