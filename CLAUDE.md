...
- **签名原理 + dev 稳定签名设置 + 发版（Developer ID / Hardened Runtime / 公证 / tauri-action）一步步清单见 [docs/SIGNING.md](docs/SIGNING.md)。** dev 期稳定签名已落地，避免钥匙串 / 辅助功能反复授权。
...
7. [x] P2 收尾代码策略（退役 `doubao_streaming_preview_enabled` 设置/UI；豆包 `doubao_access_token` 配好即默认尝试流式，未配或流式启动失败仍走 P1 批量；不引入 provider priority/routing 新抽象，不新增 command/event，不 emit `partial-transcript`；`PROJECT_SPEC.md` §5.1/§5.4 已同步真实状态；`cargo fmt --check`、`cargo test`、`cargo clippy -- -D warnings`、`pnpm exec tsc --noEmit`、`pnpm build` 通过；5 次真实「松手→注入完成」延迟仍需在 App + 豆包 key 下手动验收后填入 §5.4）

**现在在做：** P3.1 Design System / Component Library v1：先在 Figma 建 Audie 自己的组件库，再基于组件库做 overlay / settings / onboarding screens；Figma 文件为 `Audie Design System v1`（https://www.figma.com/design/HwyjD0TlY1amuLcqfCvPNB）。`design.dark.md` / `design.md` 的 Geist colors / typography / spacing / rounded / component tokens 已全量整理进 Foundations（Figma 缺 Geist 字体时 text styles 暂回退 Inter，但保留 Geist token 名称和指标）。本切片不接 `daisyui-blueprint`，不复制付费 Blueprint 内容，不改 `src/` / `src-tauri/`。

> P2.3–P2.7 切片表已按「松手即出 / 不要滚动字幕 / fn 留 P3」同步到 `PROJECT_SPEC.md` §5.1。P2.7 代码收口完成；真实延迟数据还没有手动落档，但不阻塞 P3。不要顺手开多 provider priority/routing，那个要先补 SPEC 另开切片。P3.1 后下一片应是 P3.2 Overlay 胶囊视觉打磨。
