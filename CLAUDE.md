...
4. [ ] 豆包配置 UI + keychain + overlay 文案清理（设置页 4 字段 AppID/Token/endpoint/resource_id；Access Token 走 keychain key id `doubao_access_token`；新增 `doubao_streaming_preview_enabled: bool` 默认 false；overlay RECORDING 态去掉「录音中…」占位文字只剩波形；不进 `list_asr_providers`）
5. [ ] AudioManager PCM chunk 实时出口 + dev-only 离线连通性 command（cpal callback 多一路 broadcast，批量 buffer 不变；新 `asr/doubao/client.rs` ws 客户端；dev command `test_doubao_streaming(wav_path)`；新依赖 `tokio-tungstenite` + `tokio-rustls`）
6. [ ] 录音热路径接流式（开关开 + 豆包配好 → 录音时并行送豆包 ws，松手用豆包 final 替代批量 → 润色 → 注入；开关关或未配 → 走 P1 批量；不 emit `partial-transcript`，不动 overlay）
7. [ ] P2 收尾（豆包配好默认走流式，未配批量降级；记录 5 次延迟到 SPEC §5.4；同步 `PROJECT_SPEC.md` §5.1 切片表）

**现在在做：** P2.4 豆包配置 UI + keychain + overlay 文案清理。
