...
**现在在做：** P0.3 待开（VAD 断句，Silero 抄 Handy）。P0.2 收尾遗留：
  - 3 根柱状体只是占位，typeless 那种 N 根条形阵列留到 P3 体验打磨
  - 麦克风选择 (`list_microphones`) 是 P0.6 的活，当前固定默认设备
  - cpal 失败时（无权限 / 无设备）目前只 log，UI 上柱状体不动；P0.7 才把 `AppError::Permission` / `Device` 推到 `error` event 让 overlay 变红
