...
3. [x] BYOK 设置页 + provider test（设置页新增 Groq / OpenAI / OpenAICompatible 三块 key 表单，写入仍走 `set_secret` 到系统 keychain，已保存 key 不回填明文；新增 `test_provider` command 从 keychain 读 `key_id` 后探测 `/models` 可达性，成功/失败在设置页明确反馈；reqwest timeout/connect 归 `Network`，401 归 `Provider`，403/429/5xx 归 `Network`，不联网时给「网络失败」错误）
...
2. [ ] 给 ASR provider 增加流式合同，但先不接真实 provider（Rust 里有 `transcribe_stream` 的类型/事件骨架；现有批量 Groq/OpenAI/WhisperCpp 链路不变；测试能证明批量路径没被破坏）
3. [ ] 接入第一个真实 WebSocket 流式 ASR provider（选定一个 provider 后，按住说话时后端能收到 partial/final；先打印日志或发事件，不改注入策略）
4. [ ] 录音边采集边上传 chunk（不再等松手后整段上传；录音时音频 chunk 持续送进 stream；松手后关闭流并拿 final）
5. [ ] Overlay 显示实时 partial transcript（胶囊 RECORDING 态从「录音中…」变成实时字幕；partial 抖动可接受，但不能乱序或闪空）
6. [ ] 松手即 final 注入 + P2 收尾（松手后用 final transcript 走现有润色/注入链路；完整体验达到「边说边看字幕，松手后很快出文字」）

**现在在做：** P2.2 ASR provider 流式合同骨架（先不接真实 provider）。
