...
- 写系统层（快捷键/注入/VAD）时，明确让 AI"先看 Handy 对应实现，按它的模式来"，再动手。
...
- [ ] **P0** 跑通核心链路（复刻 Handy 最小集）：快捷键→录音→VAD→批量转写→剪贴板注入→设置持久化
...
**现在在做：** P0.2 待开。P0.1 收尾遗留：
  - 单键 fn / 双击 Option 触发已挪到 P3（SPEC §5 那行新加的「用户最期待的触发方式」）；P0 不动 CGEventTap
  - 主窗的 `state:` 徽章只是 debug 用，P0.2+ 想替换/移除随时可以
  - 透明 overlay 走 `macOSPrivateApi: true`（不上 App Store 才能用，目前是开源/portfolio 无所谓）