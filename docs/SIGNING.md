...
### Debug 与 Release 的 Keychain 条目分开

开发证书与打包 App 的签名身份可能不同。即使 bundle id 相同，共用一条 Keychain 记录也会让 macOS 在另一身份读取明文时要求验证。因此：

- Release service 保持 `com.audie.app.secure-storage`，不迁移、不删除已有 key。
- 启用 `debug_assertions` 的构建使用 `com.audie.app.dev.secure-storage`；Debug 首次需重新填写一次 key。
- `tauri build --debug` 也启用 `debug_assertions`，所以同样使用 Debug service。
- 分 service 只解决 Debug/Release 身份互相冲突；Debug 跨重启、重编静默读取仍以前面的稳定 Apple Development 签名为前提。
...
### 省钱版：不加入 Apple 开发者计划（$99 不想付）

不付费就没有 Developer ID、没法公证。后果：别人从 GitHub 下载你的 app，macOS 给它打上 `com.apple.quarantine`（隔离）标记，Gatekeeper 直接拦——提示「**已损坏，无法打开**」或「**无法验证开发者**」。

**app 本身没坏，是被那个隔离标记挡了。让用户自己去掉标记就能开。** 两条路，写进你的 README / Release 说明告诉用户：

**A. 右键打开**（最简单，对「无法验证开发者」有效）
访达里**右键** app → 打开 → 弹窗里再点「打开」。macOS 记住后，以后双击就行。

**B. `xattr` 去隔离标记**（对「已损坏、右键也打不开」有效）
让用户在终端跑（app 装哪就写哪个路径）：
```bash
xattr -cr /Applications/Audie.app
# 或只删隔离这一个属性：
xattr -d com.apple.quarantine /Applications/Audie.app
```
- `-cr` = clear recursive，把所有扩展属性递归清掉（含隔离标记）。跑完正常双击打开。
- 原理：从网上下载的文件都带 `com.apple.quarantine`，Gatekeeper 见它+没公证就拦；删掉这个属性 = 告诉系统「这是我主动信任的」。

> ⚠️ 这是 **workaround，不是正经发版**。让普通用户跑终端命令很劝退、也显得不正规，只适合早期开源 / 技术向用户。真要面向大众，还是得走 Developer ID + 公证。
> ❌ 别让用户 `sudo spctl --master-disable`（整个关掉 Gatekeeper）——太粗暴、降低他全机安全。只去单个 app 的隔离标记就够。
> 💡 `tauri build` 默认会给产物 ad-hoc 签名（Apple Silicon 上不签根本跑不了），所以「已损坏」基本都是隔离标记的事，`xattr -cr` 一下就好。
...
### 当前 v0.1 自动化：release 分支 + xattr

当前 `.github/workflows/release.yml` 在 `main -> release` PR 合并后构建 Apple Silicon DMG，自动创建 tag 和 GitHub Pre-release。构建时显式设置 `signingIdentity: "-"`，让 `.app` 整体获得可由 `codesign --verify --deep --strict` 验证的 ad-hoc 签名；这仍然不是 Developer ID 签名。产物未经 Apple 公证，因此 Release notes 必须保留以下安装说明：
...
这只是早期测试分发方案。面向普通用户前必须迁移到下面的 Developer ID + 公证流程，并从说明中移除 `xattr`。

### 正式自动化：GitHub Actions + tauri-action（对应项目 P4）
...
6. [ ] 将现有 `.github/workflows/release.yml` 从 ad-hoc 构建升级为 Developer ID 签名与公证。
7. [ ] 证书 .p12 / 各密码 / minisign 私钥 全进 **GitHub Secrets**。
8. [ ] 合并 `main -> release`，确认 Actions 自动创建 tag 和无需 `xattr` 的正式 Release。
...
| 隔离标记 / quarantine | 从网上下载的文件被打的标，Gatekeeper 见它+没公证就拦；`xattr -cr <app>` 清掉就能开 |
